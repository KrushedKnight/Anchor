...
enum UserProfileTuner {
    ...
    static func tunedInterventionConfig(
        ...
        if softRate > 0.65 {
            config.softCooldown = clamp(base.softCooldown * 1.5, min: 8, max: 30)
        } else if softRate < 0.3 && profile.softInterventionsFired >= 10 {
            config.softCooldown = clamp(base.softCooldown * 0.7, min: 5, max: 30)
        }
        ...
        if strongRate < 0.3 && profile.strongInterventionsFired >= 5 {
            config.strongCooldown = clamp(base.strongCooldown * 1.5, min: 15, max: 60)
        } else if strongRate > 0.6 {
            config.strongCooldown = clamp(base.strongCooldown * 0.8, min: 12, max: 60)
        }

        // Escalation delay: if soft works well, wait longer before escalating.
        // If soft fails but strong works, escalate faster.
        if softRate > 0.6 {
            config.escalationDelay = clamp(base.escalationDelay * 1.5, min: 10, max: 45)
        } else if softRate < 0.3 && strongRate > 0.5 && profile.softInterventionsFired >= 10 {
            config.escalationDelay = clamp(base.escalationDelay * 0.7, min: 8, max: 45)
        }
